...
def prometheus_metrics(db: Database = Depends(get_db)) -> PlainTextResponse:
    ...
    # contributors total
    try:
        r = db.conn.execute("SELECT COUNT(1) FROM contributors").fetchone()
    ...
    try:
        r = db.conn.execute("SELECT COUNT(1) FROM events WHERE type = 'brain.cycle.v1'").fetchone()
    ...
    # karma intents total
    try:
        r = db.conn.execute("SELECT COUNT(1) FROM karma_intents").fetchone()
    ...
    # karma settled total
    try:
        r = db.conn.execute("SELECT COUNT(1) FROM karma_intents WHERE settled = 1").fetchone()
    ...
    try:
        r = db.conn.execute("SELECT COUNT(1) FROM events WHERE type LIKE 'signal.%'").fetchone()
    ...
    # positions total
    try:
        r = db.conn.execute("SELECT COUNT(1) FROM positions").fetchone()
    ...
    try:
        r = db.conn.execute("SELECT COALESCE(SUM(request_count), 0) FROM rate_limit_windows").fetchone()
