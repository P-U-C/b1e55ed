...
def test_dashboard_fast_verify_message_mentions_partial_scope(tmp_path) -> None:
    ...
    db_path.touch()  # Must exist for route to proceed

    # Mock the Database class entirely to avoid schema migration issues
    mock_db = MagicMock()
    mock_cursor = MagicMock()
    mock_cursor.fetchone.return_value = (42,)  # simulated event count
    mock_db.conn.execute.return_value = mock_cursor
    mock_db.verify_hash_chain.return_value = True

    with (
        patch("dashboard.app._get_brain_db", return_value=db_path),
        patch(
            "engine.core.database.Database",
