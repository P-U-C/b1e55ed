...
class TestDBMigration:
    ...
    def test_agent_id_is_nullable(self):
        ...
        db.conn.commit()
        row = db.fetchone("SELECT agent_id, chain_tx_hash FROM contributors WHERE id = ?", ("test-1",))
...
class TestContributorRegistryChainWiring:
    ...
    def test_register_with_chain_client_sets_agent_id(self):
        ...
        row = db.fetchone("SELECT agent_id FROM contributors WHERE node_id = ?", ("node-onchain",))
