...
class TestKarmaChainQueueTable:
    ...
    def test_default_status_is_pending(self):
        ...
        db.conn.commit()
        row = db.fetchone("SELECT status FROM karma_chain_queue WHERE forecast_id = ?", ("fc-001",))
...
class TestKarmaChainWriterNoOp:
    def test_queue_noop_when_no_chain_client(self):
        ...
        row = db.fetchone("SELECT COUNT(*) as cnt FROM karma_chain_queue")
        assert row["cnt"] == 0
...
class TestKarmaChainWriterWithMock:
    ...
    def test_flush_marks_submitted_in_db(self):
        ...
        row = db.fetchone("SELECT status, tx_hash FROM karma_chain_queue WHERE forecast_id = ?", ("fc-sub-001",))
    ...
    def test_flush_marks_failed_on_none_return(self):
        ...
        assert tx_hashes == []
        row = db.fetchone("SELECT status FROM karma_chain_queue WHERE forecast_id = ?", ("fc-fail-001",))
